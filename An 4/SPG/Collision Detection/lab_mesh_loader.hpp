...
namespace lab{
    ...
    //definitie forward
    void _loadObjFile(const std::string &filename, std::vector<VertexFormat> &vertices, std::vector<unsigned int> &indices, float xOff, float yOff, float zOff);

    //incarca un fisier de tip Obj (fara NURBS, fara materiale) cu deplasamentul pe x, y, z
    ...
    void loadObj(const std::string &filename, Mesh& mesh, float xOff, float yOff, float zOff, std::vector<VertexFormat> &vertices){

        //incarca vertecsii si indecsii din fisier
        std::vector<unsigned int> indices;
        _loadObjFile(filename, vertices, indices, xOff, yOff, zOff);
    ...
    void _loadObjFile(const std::string &filename, std::vector<VertexFormat> &vertices, std::vector<unsigned int> &indices, float xOff, float yOff, float zOff){
        ...
        while (std::getline(file, line)){
            ...
            //daca am un vertex
            if (tokens.size()>3 && tokens[0] == "v") positions.push_back(glm::vec3(_stringToFloat(tokens[1]) + xOff, _stringToFloat(tokens[2]) + yOff, _stringToFloat(tokens[3]) + zOff));
