...
-}
filterLC :: (a -> Bool) -> [a] -> [a]
filterLC f list = [a | a <- list, f a == True]
...
-}
multime = [(a, b, cmmdc a b) | a <- [1..10], b<-[2..10], b > a]
...
firstWord input = if((head input) == ' ') 
                    then []
                  else
                    [(head input)] ++ firstWord (tail input)

-- Verificare: check7
check7 = firstWord "Cel laborator frumos!" == "Cel"
...
subst what new input = if((head (words input)) == what)
                        then unwords(new : (tail (words input)))
                      else
                        ((head (words input)) ++ " ") ++ (subst what new (unwords (tail (words input))))
...
-}
increasingPairs = [(x, y) |  s <- [0..], x <- [0..s], y <- [0..s], x < y, (x + y) == s]
