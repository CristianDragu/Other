...
randoms :: Int -> [Word8]
randoms = map (fromIntegral . fst) . iterate (next . snd) . next . mkStdGen
...
substCrypt = map --(\x -> f x) ruleaza automat pe aceasta multime.
...
xorCrypt s = zipWith (\x y -> (xor (charToWord x) y)) s
...
test_xor2 :: IO ()
test_xor2 = assertEqual
  (xorCrypt2 "This is a super secret string!" $ take 3 $ repeat 2)
  [86,106,107,113,34,107,113,34,99,34,113,119,114,103,112,34,113,103,97,112,
   103,118,34,113,118,112,107,108,101,35]
...
xorCrypt2 s k = zipWith (\x y -> (xor (charToWord x) y)) s (take (length k) $ cycle k)
