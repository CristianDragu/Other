...
void IO_init(void)
{
	DDRB &= ~((1 << PB1) | (1 << PB0));	// PB0, PB1 ca input
	PORTB |= (1 << PB1) | (1 << PB0);	// pull-up-uri pentru PB0 si PB1
}
...
/*
 * Listeaza pe LCD continutul directorului primit ca parametru
 */
void list(const char *path)
{
	// TODO 1 - afisati pe LCD continutul directorului primit ca parametru
	//  -folositi functia pf_opendir pentru a deschide directorul pentru iteratie
	//  -folositi functia pf_readdir pentru a itera prin continutul directorului
	//  -ATENTIE!!! functia pf_readdir nu returneaza un cod de eroare cand ajunge
	//	la sfarsitul directorului, ci returneaza un fisier cu nume vid (stringul "")
	//  -folositi prima linie a LCD-ului (adresa 0x00) pentru afisare si un
	//	delay de 200ms
	(void)path;
	DIR d;
	FILINFO f;
	FRESULT ok = pf_opendir(&d, path);
	do{
		pf_readdir(&d, &f);
		if(strlen(f.fname) <= 1)
			break;
		LCD_printAt(0x00, f.fname);
		_delay_ms(200);
		LCD_clear_top_line();
	}while(1);
...
/*
 * Citeste si salveaza numele primelor 2 fisiere cu extensia ".WAV" din folderul primit
 */
void get_music(const char *path)
{
	// scanam doar daca nu am salvat deja numele fisierelor
	if(found_files > 0)
		return;

	// TODO 2 - scanati directorul primit dupa fisiere .WAV
	//  - folositi campul fattrib pentru a distinge intre fisiere si directoare
	//  	(fisierele _nu_ au bitii AM_DIR sau AM_HID setati)
	//  - atentie la conditia de terminare a iteratiei (nume vid)
	//  - salvati denumirile primului (bonus - primelor 2) fisiere care indeplinesc
	//	conditiile, pentru a fi folosite mai tarziu
	//  - incrementati variabila found_files pentru fiecare fisier gasit
	(void)path;
	...
	if((PINB & (1 << PB0)) == 0){
		do{
			pf_readdir(&d, &f);
			if(strlen(f.fname) <= 1 || f.fattrib == AM_HID || f.fattrib == AM_DIR || strstr(f.fname, ".WAV") == NULL)
				continue;
			char *cale;
			cale = (char*)malloc(100 * sizeof(char));
			strcpy(cale, path);
			strcat(cale, "/");
			strcat(cale, f.fname);
			LCD_clear_top_line();
...
void task2()
{
	get_music(music_folder);

	// TODO 2 - porniti redarea audio daca unul din butoane e apasat
	//  - folositi functia play cu path-ul absolut al fisierului salvat
	//  - redati primul fisier daca este apasat butonul PB0
	//  - bonus - redati al doilea fisier daca este apasat butonul PB1
	//  - afisati pe LCD numele fisierului redat
}
...
const char speech_folder[] = "speech";
...
void task4()
{
	if((PINB & (1 << PB0)) == 0 && (PINB & (1 << PB1)) == 0)
	{
		...
		DIR dir;
		if(pf_opendir(&dir, speech_folder) != FR_OK)
		{
