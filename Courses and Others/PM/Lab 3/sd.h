...
/* Definitii accesul SPI la cardul SD */
#define SD_PORT		PORTD
#define SD_DDR		DDRD
#define SD_CS		6

/* Controlul portului */
#define SD_SELECT()	SD_PORT &= ~(1<<SD_CS)	/* SD CS = 0 */
#define SD_DESELECT()	SD_PORT |=  (1<<SD_CS)	/* SD CS = 1 */
#define	SD_SELECTED()	!(SPI_PORT &  0x10)	/* SD CS status (1 = selectat; 0 = deselectat) */

#define SPI_DELAY()	__builtin_avr_delay_cycles(2500 / (1000000000/F_CPU))
